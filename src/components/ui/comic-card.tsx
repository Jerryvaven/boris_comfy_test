...
export function ComicCard({
  ...
  return (
    <div className="bg-gray-800 min-w-[260px] max-w-[260px] rounded-lg pl-2 pr-2 pb-2 pt-0 border-2 border-yellow-400 flex flex-col flex-shrink-0 relative hover:shadow-2xl hover:-translate-y-3 hover:scale-105 transition-all duration-300 ease-out cursor-pointer mt-8">
      {/* Yellow Banner */}
      <div 
        className={`bg-yellow-400 text-black -ml-2 -mr-2 mt-0 px-2 py-3 rounded-t-lg flex justify-between items-center ${
          onToggleOverlay ? 'cursor-pointer hover:bg-yellow-300 transition-colors' : ''
        }`}
      ...
      {/* Overlay */}
      {showOverlay && (
        <div className="absolute inset-0 bg-yellow-400 rounded-lg flex flex-col text-black overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto px-3 pb-3 space-y-2">
            <p className="text-sm">[Sep 2025]</p>
            <p className="text-sm">Episode {number}: Supercal Freaky</p>
            <p className="text-sm">
              by{" "}
              <Link
                href={`/author/${author.toLowerCase().replace(/\s+/g, "-")}`}
          ...
          <div className="p-3 pt-2 border-t border-black/10">
            {quantity > 0 ? (
              <div className="flex items-center justify-between">
                <span className="text-black font-semibold">Added to Cart</span>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => onUpdateQuantity?.(-1)}
                    className="text-yellow-600 hover:bg-yellow-500 hover:text-black rounded px-2 py-1 transition-colors"
                  >
                    <Minus size={14} />
                  </button>
                  <span className="text-black font-semibold min-w-[20px] text-center">{quantity}</span>
                  <button
                    onClick={() => onUpdateQuantity?.(1)}
                    className="text-yellow-600 hover:bg-yellow-500 hover:text-black rounded px-2 py-1 transition-colors"
                  >
              ...
              <button
                onClick={onAddToCart}
                className="w-full bg-black text-yellow-400 px-4 py-2 rounded font-semibold hover:bg-gray-800 transition-colors"
              >
                Add to Cart
              </button>
            )}
